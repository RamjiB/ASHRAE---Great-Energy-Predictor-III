...
cat_feats = ['building_id', 'primary_use', 'meter', 'weekday', 'hour']
...
for site_id in tqdm(range(16), desc="site_id"):
    ...
    for fold,(trn_idx,val_idx) in enumerate(kf.split(x_train_site,y_train_site)):
        ...
        cat_model.fit(x_tr,y_tr,eval_set=(x_va,y_va),use_best_model=True,verbose = 101)
        models[site_id].append(cat_model)
        y_pred_val = cat_model.predict(x_va)
...
del train_df, x_train_site, y_train_site, x_tr, y_tr, x_va, y_va, y_pred_train_site, y_pred_val, rmse, score, cv_scores
gc.collect()
...
for site_id in tqdm(range(16), desc='site_id'):
    ...
    for fold in range(cv):
        cat_model = models[site_id][fold]
        y_pred_test_site += cat_model.predict(x_test_site) / cv
        gc.collect()
...
sub.to_csv('learning_cat_submission.csv', index=False)
